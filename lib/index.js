            ...
            cssLoader(sourceFile, staticPath)
              .then(function(ret){
                try{
                  ...
                  fs.writeFileSync(dumpUtil.relative(item.rep), ret.css);
                }catch(e){
                  throw new Error(e);
                }
                resolve();
  ...
  compiler.plugin('done', function(statsData, callback) {
    ...
    templateFiles.forEach(function(templateFile){
      ...
      curDependencies.forEach(function(item){
        ...
        sourceFile = dumpUtil.relative(isPreprocessed ? item.rep : item.dep);
        // fallback if can't find it in source folder.
        if(sourceFile == null){
